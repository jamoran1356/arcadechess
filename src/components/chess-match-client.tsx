    ...
    startTransition(() => {
      void fetch(`/api/matches/${match.id}/move`, {
        ...
        .then(async (response) => {
          ...
          if (!response.ok) {
            throw new Error(data.error ?? "No se pudo procesar el movimiento.");
          }
        ...
        .catch((requestError: unknown) => {
          setError(requestError instanceof Error ? requestError.message : "No se pudo procesar el movimiento.");
        })
        .finally(() => {
          setIsPending(false);
        });
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
      <section className="panel rounded-[2rem] p-4 sm:p-6">
        <div className="flex items-center justify-between gap-4 pb-4">
          <div>
            <p className="eyebrow">Mesa en vivo</p>
        ...
        <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/85 p-3">
          <Chessboard
            ...
            customDarkSquareStyle={{ backgroundColor: "#0f3857" }}
            customLightSquareStyle={{ backgroundColor: "#d6f0ff" }}
            customBoardStyle={{ borderRadius: "1.25rem", overflow: "hidden" }}
          />
        </div>

        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-slate-400">Blancas</p>
            <p className="mt-2 text-lg font-semibold text-white">{match.host.name}</p>
          </div>
          <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-slate-400">Estado</p>
            <p className="mt-2 text-lg font-semibold text-white">{status.replaceAll("_", " ")}</p>
          </div>
          <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-slate-400">Negras</p>
            <p className="mt-2 text-lg font-semibold text-white">{match.guest?.name ?? (match.isSolo ? "Modo solo" : "Esperando rival")}</p>
          </div>
        </div>
      ...
      <aside className="grid gap-5 self-start">
        ...
        <div className="panel rounded-[2rem] p-6">
          <p className="eyebrow">Historial SAN</p>
        ...
        <div className="panel rounded-[2rem] p-6">
          <p className="eyebrow">Estado de mesa</p>
