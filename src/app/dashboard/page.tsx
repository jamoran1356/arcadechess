...
import { getDashboardSnapshot, getUserRankPosition } from "@/lib/data";
...
export default async function DashboardPage() {
  ...
  const [user, rawFriendships, enabledNetworks, rankInfo] = await Promise.all([
    ...
    getEnabledNetworks(),
    getUserRankPosition(session.id),
  ]);
            ...
            <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
              ...
              <div className="mt-4 grid gap-3">
                <div className="flex items-center justify-between text-sm text-slate-300">
                  ...
                  <span className="text-lg font-semibold text-amber-200">{rankInfo.position ? `#${rankInfo.position}` : "—"}</span>
                </div>
                <div className="flex items-center justify-between text-sm text-slate-300">
